...
if(get_post_type() == 'event') {
	$terms = wp_get_post_terms(get_the_ID(), 'event_categories', ['fields' => 'ids']);
} else {
...
$related_posts_args_by_tags = [
	...
	'tax_query' => [
		...
		[
			'taxonomy' => 'post_tag',
			'field' => 'term_id',
			'terms' => [269], // exclude unlisted tag
			'operator' => 'NOT IN'
		]
	]
];
	...
	$related_posts_args_by_terms = [
		...
		'tax_query' => [
			...
			[
				'taxonomy' => $term_tax,
				'field' => 'term_id',
				'terms' => [225, 226], // exclude Private categories
				'operator' => 'NOT IN',
			],
			[
				'taxonomy' => 'post_tag',
