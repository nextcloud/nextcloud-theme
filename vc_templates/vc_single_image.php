...
if (empty($onclick) && isset($img_link_large) && $img_link_large === 'yes') {
	$onclick = 'img_link_large';
} elseif (empty($atts['onclick']) && (! isset($atts['img_link_large']) || $atts['img_link_large'] !== 'yes')) {
	$onclick = 'custom_link';
}
...
switch ($onclick) {
	...
	case 'link_image':
		...
		// backward compatibility
		if (! vc_has_class('prettyphoto', $el_class) && $source === 'external_link') {
			$link = $custom_src;
		} elseif (! vc_has_class('prettyphoto', $el_class)) {
			//$link = wp_get_attachment_image_src( $img_id, 'large' ); -- old
...
if (in_array($source, [ 'media_library', 'featured_image' ], true) && $add_caption === 'yes') {
